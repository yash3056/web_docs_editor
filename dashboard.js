class AdvancedDocumentDashboard {
    ...
    bindClassificationEvents() {
        const classifyBtn = document.getElementById('classify-docs-btn');
        const classificationModal = document.getElementById('classification-modal');
        const closeModalBtn = document.getElementById('close-classification-modal');
        const fileDropZone = document.getElementById('file-drop-zone');
        const fileInput = document.getElementById('pdf-file-input');
        const downloadBtn = document.getElementById('download-watermarked-btn');
        ...
        classificationModal.addEventListener('click', (e) => {
            if (e.target === classificationModal) {
                classificationModal.classList.remove('show');
            }
        });
    ...
    resetClassificationModal() {
        ...
        document.getElementById('download-watermarked-btn').style.display = 'none';
    ...
    handleFileSelection(file) {
        ...
        this.classifyDocument(file);
    }

    async classifyDocument(file) {
        const progressDiv = document.getElementById('classification-progress');
        ...
        try {
            ...
            const response = await fetch('http://localhost:25158/api/classify-and-watermark', {
                method: 'POST',
                body: formData
            });
            ...
            alert('Failed to classify document. Please ensure the classification service is running.');
        }
    }

    displayClassificationResult(result) {
        const resultDiv = document.getElementById('classification-result');
        const levelDiv = document.getElementById('classification-level');
        const confidenceFill = document.getElementById('confidence-fill');
        const confidenceText = document.getElementById('confidence-text');
        const reasoningText = document.getElementById('classification-reasoning');
        const riskFactorsList = document.getElementById('risk-factors-list');
        const recommendationsList = document.getElementById('recommendations-list');
        const downloadBtn = document.getElementById('download-watermarked-btn');

        // Set classification level
        levelDiv.textContent = result.classification;
        levelDiv.className = `classification-level ${result.classification.replace(/\s+/g, '-')}`;
        ...
        if (result.key_risk_factors && result.key_risk_factors.length > 0) {
            result.key_risk_factors.forEach(factor => {
        ...
        if (result.handling_recommendations && result.handling_recommendations.length > 0) {
            result.handling_recommendations.forEach(recommendation => {
                const li = document.createElement('li');
            ...
            const li = document.createElement('li');
            li.textContent = 'Follow standard document handling procedures.';
            recommendationsList.appendChild(li);
        }

        // Set download URL
        if (result.download_url) {
            this.currentDownloadUrl = `http://localhost:25158${result.download_url}`;
