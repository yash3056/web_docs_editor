...
class DocsEditor {
  ...
  updatePageLayout() {
    ...
    requestAnimationFrame(() => {
      ...
      // Reset the flag so that a new update can be scheduled.
      this.isUpdatingLayout = false;
    });
  }
  ...
  getCurrentParagraphElement() {
    ...
    console.log(
      "No paragraph element found, checking if editor has div children"
    );
  ...
  setElementLineSpacing(element, spacing) {
    ...
    console.log(
      `Applied line-height: ${spacing} to element:`,
      element.tagName,
      element
    );
  }
  ...
  populateLineSpacingModal() {
    ...
    if (currentElement) {
      ...
      document.getElementById("space-before").value = Math.round(
        marginTop * 0.75
      ); // Convert px to pt approximately
      document.getElementById("space-after").value = Math.round(
        marginBottom * 0.75
      );
    }
  ...
  setupLineSpacingModalEvents() {
    ...
    document
      .getElementById("line-spacing-type")
    ...
    // Modal buttons
    document
      .getElementById("apply-spacing-btn")
    ...
    document
      .getElementById("cancel-spacing-btn")
      .addEventListener("click", () => {
        this.closeLineSpacingModal();
      });
  ...
  updateSpacingPreview() {
    ...
    const value =
      parseFloat(document.getElementById("line-spacing-value").value) || 1.0;
    const spaceBefore =
      parseFloat(document.getElementById("space-before").value) || 0;
    const spaceAfter =
      parseFloat(document.getElementById("space-after").value) || 0;
  ...
  applyDetailedSpacing() {
    ...
    if (selection.rangeCount === 0) {
      const currentElement = this.getCurrentParagraphElement();
      if (currentElement) {
        this.applyDetailedSpacingToElement(
          currentElement,
          lineHeight,
          spaceBefore,
          spaceAfter
  ...
  updateWordCount() {
    ...
    document.getElementById(
      "char-count"
    ).textContent = `Characters: ${characters}`;
  }
    ...
    if (menu.classList.contains("show")) {
      setTimeout(() => {
        document.addEventListener(
          "click",
          this.closeLineSpacingMenu.bind(this),
          { once: true }
        );
      }, 0);
    }
  ...
  setupModalEvents() {
    ...
    document
      .getElementById("apply-watermark-btn")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("remove-watermark-btn")
      .addEventListener("click", () => {
        this.removeWatermark();
        document.getElementById("watermark-modal").style.display = "none";
    ...
    document
      .getElementById("cancel-watermark-btn")
      .addEventListener("click", () => {
        document.getElementById("watermark-modal").style.display = "none";
      });
  }
  ...
  async testCanvasWatermark() {
    try {
      ...
      console.log(
        "Canvas watermark test completed. Check the preview in the top-right corner."
      );
      this.showNotification(
  ...
  async saveDocumentWithVersion(commitMessage = null, showNotification = true) {
    ...
    try {
      ...
      if (
        ...
        if (showNotification) {
          if (!hasContentChanged && !hasWatermarkChanged) {
            console.log(
              "📄 No content or watermark changes detected, skipping version creation"
            );
            this.showNotification("No changes to save", "info");
          } else if (isPlaceholderContent) {
            console.log(
              "📄 Placeholder content detected, skipping version creation"
            );
      ...
      if (!currentDocId && isPlaceholderContent) {
        if (showNotification) {
          console.log(
            "📄 Cannot save new document with only placeholder content"
          );
          this.showNotification(
      ...
      if (this.watermarkSettings) {
        ...
        console.log(
          "Including watermark in document save:",
          this.watermarkSettings
        );
      }
      ...
      try {
        ...
        if (result && result.success) {
          ...
          const currentWatermarkHash = this.watermarkSettings
            ...
            : "";
          localStorage.setItem(
            "lastWatermarkHash_" + document.id,
          ...
          if (showNotification) {
            this.showNotification(
              "Document saved with version control!",
              "success"
            );
          }
        ...
        if (showNotification) {
          this.showNotification(
            "Document saved locally (server error)",
            "warning"
          );
        }
...
<body>
  ...
  async exportAsDOCX() {
    try {
      ...
      if (typeof docx === "undefined" || !docx) {
        ...
        this.showNotification(
          "DOCX library not available. Please refresh the page and try again.",
          "error"
        );
        return;
      }
  ...
  elaborateText(text) {
    return (
      text +
      " Furthermore, this approach provides additional benefits and considerations that enhance the overall effectiveness and impact of the implementation."
    );
  }
